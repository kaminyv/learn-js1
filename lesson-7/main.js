...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Функция добавления рекордов
    function gameRecord() {
        console.log('Record');
        if (record.length == 0) {
            document.getElementById('record').innerHTML = '<h2>Таблица рекордов</h2>';
        } else {
            document.getElementById('record').innerHTML = '<h2>Таблица рекордов</h2>' + '<table><tbody><tr><th>Счет</th><th>Линий собрано</th></tr></tbody></table>';
            var tbody = document.getElementsByTagName('tbody')[0];
            for (var i = 0; i < record.length; i++) {
                var tr = document.createElement('tr');
                for (key in record[i]) {
                    var td = document.createElement('td');
                    td.appendChild(document.createTextNode(record[i][key]));
                    tr.appendChild(td);
                }
    ...
    function gameOver() {
        ...
        gameFinished = true;
        record.push({ score: Number(gameBoard.score), flines: Number(gameBoard.finishedLines) });
        gameRecord();
    }
